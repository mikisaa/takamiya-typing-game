...
### 7. UI デザイン & アイコン契約
* **5色パレット**: `#FFFFFF`, `#F5FBDA`, `#D9EFBD`, `#B9D175`, `#450C3F` を厳格に遵守。
* **Zero OS Emoji**: Unicode / OS 絵文字を完全排除。
* **Bespoke Pixel Inline SVG**: モード選択カード（本番トラック / 練習キーボード）、ランキング、ステージバッジ、SUCCESS/MISSフィードバック、リザルト指標に自作ピクセルインラインSVGを配置。文字代替（`[本]`、`[練]`）を完全撤廃。
