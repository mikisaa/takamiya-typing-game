...
- [TASK-001] 開発環境 & プロジェクトベース構成の準備 @antigravity [LIFECYCLE:PREFLIGHT]
  - ディレクトリ構造（`src/`, `src/engine/`, `src/assets/`, `src/services/`, `src/config/`, `src/gas/`）の整備
...
- [TASK-002] ゲーム設定 & 初期データモデルの実装 ([FR-014], [FR-016]) @antigravity [LIFECYCLE:IMPLEMENTATION]
  - `src/config/gameConfig.js`（速度、制限時間、ペナルティ、ボーナス閾値）の実装
  - `src/data/defaultQuestions.js`（難易度別タイピング問題プール）の実装

- [TASK-003] タイピング入力 & ローマ字揺れ判定エンジンの実装 ([FR-002]) @antigravity [LIFECYCLE:IMPLEMENTATION]
  - `src/engine/typingMatcher.js`（Keydown直接捕捉、Prefix-treeローマ字マッチング、複数表記ゆれ判定、ミスタイプ検知）の実装

- [TASK-004] 2Dゲームループ & スコア・コンボ制御エンジンの実装 ([FR-001], [FR-010], [FR-011]) @antigravity [LIFECYCLE:IMPLEMENTATION]
  - `src/engine/gameEngine.js`（`requestAnimationFrame` 60fpsループ、90秒タイマー、COMBO加算、TIME BONUS、スコア計算、リザルトデータ生成）の実装
...
- [TASK-006] Pixel Artスプライト & 積込・接触アニメーションの実装 ([FR-003], [FR-004], [FR-007]) @antigravity [LIFECYCLE:IMPLEMENTATION]
  - `src/assets/pixelSprites.js`（軽トラ・4tユニック・15tユニック、7種足場資材、フォークリフト）の実装
  - `src/engine/animationController.js`（走行、SUCCESS積込アニメーション、MISS振動・資材落下アニメーション）の実装
...
- [TASK-009] タイピング判定 & スコア計算ロジック単体テスト ([TC-002], [TC-010], [TC-011], [TC-014], [TC-016]) @antigravity [LIFECYCLE:TEST]
  - ローマ字揺れ判定、ミスタイプ検出、コンボ倍率・TIME BONUS、スコア算出、Config変更反映の自動テスト実行

- [TASK-010] アニメーション制御 & 状態遷移テスト ([TC-001], [TC-003], [TC-004], [TC-005], [TC-007], [TC-008], [TC-009], [TC-017]) @antigravity [LIFECYCLE:TEST]
  - フォークリフト走行、成功/失敗アニメーション、難易度別車両、7種資材、背景進化、EXTRA動的演出、60fpsフレームレート安定性の検証
...
- [TASK-014] ブラウザUI結合 & 全体受入シナリオ検証 ([TC-006], [TC-020]) @antigravity [LIFECYCLE:ACCEPTANCE]
  - PCブラウザ（Chrome/Edge）における練習・本番モード、リザルト、ランキング画面のレスポンシブE2E受入検証
